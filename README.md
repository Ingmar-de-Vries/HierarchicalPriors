...
Note that the larger data files belonging to this repository are stored on a public OSF repository (DOI: https://doi.org/10.17605/OSF.IO/WJMVR); or look for Ingmar de Vries - HierarchicalPriors). The OSF repository includes: 
...
The code in this GitHub repository is structured as follows:
  ...
  -	Create model RDMs
    ...
    - RDMs based on video data, kinematic marker data, and eyetracker data are created outside of the main "HierarchicalPriors_dRSA.m" script (see section "Run dRSA analysis" below) at 100 Hz. The pre-created RDMs therefore have size 14x14x500x500 (i.e., stim1 x stim2 x timestim1 x timestim2). This is done to save computation time in the dRSA pipeline, i.e., the RDMs only need to be loaded in, not computed each time the dRSA script is run. 
  ...
  - Run dRSA analysis, statistics, and plotting
    - In the "dynamicRSA" subdirectory, you'll find the following scripts:
      ...
      - "HierarchicalPriors_PLOT.m" - plot ROI-based results and prepare individual-participant peak magnitudes for statistics in JASP. 
