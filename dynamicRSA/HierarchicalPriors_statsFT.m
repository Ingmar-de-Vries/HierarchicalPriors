function [stat,signpos,signneg] = HierarchicalPriors_statsFT(powspctrm,addinfo,template,testtype,pthresh,pthreshclust,baseline)
...
%now set testtype specific parameters
if contains(testtype,'timeseries')% let fieldtrip average over the sensors, so test is only run over the dimension time
    ...
    %create datastruct of zeros to test observed values against (in reality
    %they're tested against the baseline, since this data is dB baseline
    %corrected)
    datazero = data4stat;
    for isub=1:subnum
...
elseif contains(testtype,'timefreq')% let fieldtrip average over the sensors, so test is only run over the dimensions time and frequency
